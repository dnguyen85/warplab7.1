...
int baseband_processCmd(const wl_cmdHdr* cmdHdr,const void* cmdArgs, wl_respHdr* respHdr,void* respArgs, void* pktSrc){
	...
	unsigned int rfsel,buffSel,currChecksum,startSamp,currSamp,startByte,numSamp,sampLen,numPkts,i;
	...
	switch(cmdID){
		...
		case BB_WRITE_IQ:
			...
			if(flags&&SAMPHDR_FLAG_CHKSUMRESET){
				currChecksum = baseband_updateChecksum(newchkinput_16,1);
			}else{
...
#ifdef WARP_HW_VER_v2
			if(buffSel&BB_A) memcpy((void*)(WARPLAB_IQ_TX_BUFFA) + startSamp*sizeof(wl_samp),(void *)sampAddr,sampLen);
			if(buffSel&BB_B) memcpy((void*)(WARPLAB_IQ_TX_BUFFB) + startSamp*sizeof(wl_samp),(void *)sampAddr,sampLen);
			if(buffSel&BB_C) memcpy((void*)(WARPLAB_IQ_TX_BUFFC) + startSamp*sizeof(wl_samp),(void *)sampAddr,sampLen);
			if(buffSel&BB_D) memcpy((void*)(WARPLAB_IQ_TX_BUFFD) + startSamp*sizeof(wl_samp),(void *)sampAddr,sampLen);
#endif

#ifdef WARP_HW_VER_v3
			if(buffSel&BB_A) {
				while(XAxiCdma_IsBusy(&cdma_inst)) {}
				XAxiCdma_SimpleTransfer(&cdma_inst, (u32)sampAddr, (u32)((WARPLAB_IQ_TX_BUFFA) + startSamp*sizeof(wl_samp)), sampLen, NULL, NULL);
			}
			if(buffSel&BB_B) {
				while(XAxiCdma_IsBusy(&cdma_inst)) {}
				XAxiCdma_SimpleTransfer(&cdma_inst, (u32)sampAddr, (u32)((WARPLAB_IQ_TX_BUFFB) + startSamp*sizeof(wl_samp)), sampLen, NULL, NULL);
			}
			if(buffSel&BB_C) {
				while(XAxiCdma_IsBusy(&cdma_inst)) {}
				XAxiCdma_SimpleTransfer(&cdma_inst, (u32)sampAddr, (u32)((WARPLAB_IQ_TX_BUFFC) + startSamp*sizeof(wl_samp)), sampLen, NULL, NULL);
			}
			if(buffSel&BB_D) {
				while(XAxiCdma_IsBusy(&cdma_inst)) {}
				XAxiCdma_SimpleTransfer(&cdma_inst, (u32)sampAddr, (u32)((WARPLAB_IQ_TX_BUFFD) + startSamp*sizeof(wl_samp)), sampLen, NULL, NULL);
			}
#endif
		...
		case BB_READ_RSSI:
			...
			currSamp = startSamp;
			//all internal values are right...
			//xil_printf("readIQ: buffSel=0x%x startSamp=%d totalSamp=%d maxSampLen_perPkt=%d maxSamp_perPkt=%d numPkts=%d\n", buffSel, startSamp, totalSamp, maxSampLen_perPkt, maxSamp_perPkt, numPkts);
			...
			for(i=0;i<numPkts;i++){
				...
				//xil_printf("currSamp = %d, numSamp = %d\n",currSamp,numSamp);
				...
				if(cmdID == BB_READ_RSSI) {
					if(buffSel&BB_A) {
						while(XAxiCdma_IsBusy(&cdma_inst)) {}
						XAxiCdma_SimpleTransfer(&cdma_inst, (u32)(((void*)WARPLAB_RSSI_BUFFA)+startByte), (u32)((void*)respArgs + sizeof(wl_bb_sampHdr)), sampLen, NULL, NULL);
					} else if(buffSel&BB_B) {
						while(XAxiCdma_IsBusy(&cdma_inst)) {}
						XAxiCdma_SimpleTransfer(&cdma_inst, (u32)(((void*)WARPLAB_RSSI_BUFFB)+startByte), (u32)((void*)respArgs + sizeof(wl_bb_sampHdr)), sampLen, NULL, NULL);
					} else if(buffSel&BB_C) {
						while(XAxiCdma_IsBusy(&cdma_inst)) {}
						XAxiCdma_SimpleTransfer(&cdma_inst, (u32)(((void*)WARPLAB_RSSI_BUFFC)+startByte), (u32)((void*)respArgs + sizeof(wl_bb_sampHdr)), sampLen, NULL, NULL);
					} else if(buffSel&BB_D) {
						while(XAxiCdma_IsBusy(&cdma_inst)) {}
						XAxiCdma_SimpleTransfer(&cdma_inst, (u32)(((void*)WARPLAB_RSSI_BUFFD)+startByte), (u32)((void*)respArgs + sizeof(wl_bb_sampHdr)), sampLen, NULL, NULL);
					}
				}
#endif
...
int baseband_init(){
	...
	warplab_buffers_ConfigReg_Clear(
				...
				WL_BUFFERS_CFG_REG_AGC_TRIG_EN); //Don't trigger AGC
