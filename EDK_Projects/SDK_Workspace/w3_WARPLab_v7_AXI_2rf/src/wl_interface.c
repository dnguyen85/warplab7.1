...
int ifc_processCmd(const wl_cmdHdr* cmdHdr,const void* cmdArgs, wl_respHdr* respHdr,void* respArgs, void* pktSrc){
	...
	switch(cmdID){
			...
			case IFC_RX_GAIN_CTRL_SRC:
				...
				if(agc_en==0){
					...
					warplab_buffers_ConfigReg_Clear(WL_BUFFERS_CFG_REG_AGC_IQ_SEL | WL_BUFFERS_CFG_REG_AGC_TRIG_EN);
					...
					warplab_buffers_ConfigReg_Set(WL_BUFFERS_CFG_REG_AGC_IQ_SEL | WL_BUFFERS_CFG_REG_AGC_TRIG_EN);
				}
